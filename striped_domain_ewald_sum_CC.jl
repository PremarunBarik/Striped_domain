...
global z_pos_sd_upper = z_pos_sd
global z_pos_sd_lower = z_pos_sd .- 1.0

global z_pos_sd_upper_real = z_pos_sd_upper' .- z_pos_real
global z_pos_sd_lower_real = z_pos_sd_lower' .- z_pos_real

distance_upper_upper = sqrt.( ((x_pos_sd .- x_pos_sd_real).^2) .+ ((y_pos_sd .- y_pos_sd_real).^2) .+ ((z_pos_sd_upper .- z_pos_sd_upper_real).^ 2))
distance_upper_lower = sqrt.( ((x_pos_sd .- x_pos_sd_real).^2) .+ ((y_pos_sd .- y_pos_sd_real).^2) .+ ((z_pos_sd_upper .- z_pos_sd_lower_real).^ 2))
distance_lower_lower = sqrt.( ((x_pos_sd .- x_pos_sd_real).^2) .+ ((y_pos_sd .- y_pos_sd_real).^2) .+ ((z_pos_sd_lower .- z_pos_sd_lower_real).^ 2))
distance_lower_upper = sqrt.( ((x_pos_sd .- x_pos_sd_real).^2) .+ ((y_pos_sd .- y_pos_sd_real).^2) .+ ((z_pos_sd_lower .- z_pos_sd_upper_real).^ 2))
...
global reciprocal_term = (2*pi/n_x/n_y) * sum((exp.( -k_mod_2 ./ (4*(alpha^2)))) ./ k_mod_2)
...
function Ewald_sum_CC()
    ...
    term_1 = 1/2*((q_upper_upper .* erfc.(alpha .* distance_upper_upper) ./ distance_upper_upper)
                .+ (q_upper_lower .* erfc.(alpha .* distance_upper_lower) ./ distance_upper_lower)
                .+ (q_lower_lower .* erfc.(alpha .* distance_lower_lower) ./ distance_lower_lower)
                .+ (q_lower_upper .* erfc.(alpha .* distance_lower_upper) ./ distance_lower_upper))

    replace!(term_1, Inf=>0)
    term_1 = sum(term_1, dims=3) 
    term_1= vec(sum(term_1, dims=2)) ./2
            
    term_2 = (alpha/sqrt(pi)).* ((q_upper.^2) .+ (q_lower .^ 2))

    energy_ewald_sum_CC = term_1 .+ (4*reciprocal_term) .- term_2
end
