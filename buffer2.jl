...
global noise_amp_list = [5, 10, 15, 20, 25, 30, 35, 40]
global discard_percent_list = [70, 75, 80, 85, 90, 95]

global fft_peak_intensity = zeros(length(noise_amp_list), length(discard_percent_list))
for replica in 1:10
for i in eachindex(noise_amp_list)
for j in eachindex(discard_percent_list)
#introducing noise to the signal
global noise_amp = noise_amp_list[i]
...
global percent = 100 - discard_percent_list[j]
...
global bin_window = 1
global timebins = collect(0:bin_window:maximum(timestamps))
...
global AC_fft = abs.(fftshift(fft(auto_correlation)))
global AC_fft_filter = ones(length(AC_fft), 1)
global AC_fft_filter[length(AC_fft)/2+1 |>Int64] = 0 
global AC_fft_norm = sum(AC_fft .* AC_fft_filter)/(length(AC_fft)-1)

global fft_peak_intensity[i,j] += AC_fft[(maximum(delay_times)/2 + maximum(delay_times)/signal_period) + 1 |> Int64]/AC_fft_norm
...
#plot(AC_fft, minorticks=5)
#xlims!(490, 510)
#ylims!(0,10)

heatmap(fft_peak_intensity)
