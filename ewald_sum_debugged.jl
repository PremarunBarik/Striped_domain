...
function Ewald_sum_CC()
    ...
    distance_positive_i_positive_j_es[:,:, (simulation_box_num_es+1)/2 |> Int64] += diag_one
    distance_negative_i_negative_j_es[:,:, (simulation_box_num_es+1)/2 |> Int64] += diag_one

    #-----------------------------------------------------------#
    #RECIPROCAL SPACE CALCULATIONS
    global n_cut_reciprocal_es = 1
    global simulation_box_num_reciprocal = (2*n_cut_reciprocal + 1)^2


    x_pos_reciprocal_es = (2*pi)*collect(-n_cut_reciprocal:1:n_cut_reciprocal)
    x_pos_reciprocal_es = repeat(x_pos_reciprocal_es, inner=(2*n_cut_reciprocal + 1))
    x_pos_reciprocal_es = reshape(x_pos_reciprocal_es, 1, 1, simulation_box_num_reciprocal)

    y_pos_reciprocal_es = (2*pi)*collect(-n_cut_reciprocal:1:n_cut_reciprocal)
    y_pos_reciprocal_es = repeat(y_pos_reciprocal_es, outer=(2*n_cut_reciprocal + 1))
    y_pos_reciprocal_es = reshape(y_pos_reciprocal_es, 1, 1, simulation_box_num_reciprocal)

    global k_mod_2_es = (x_pos_reciprocal_es .^ 2) .+ (y_pos_reciprocal_es .^ 2)
    global k_mod_2_es[:,:, (simulation_box_num_reciprocal+1)/2 |> Int64] .= 1.0

    global exponential_term_es = exp.( - k_mod_2_es ./ (4*alpha^2)) 
    global exponential_term_es[:,:, (simulation_box_num_reciprocal+1)/2 |> Int64] .= 0.0
